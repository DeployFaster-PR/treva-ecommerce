...
import {
  ...
  NECKLACE_COLORS,
  NECKLACE_SIZES,
  NECKLACE_PRICE_RANGES,
  NECKLACE_SORT_OPTIONS,
  CATEGORY_IMAGES,
} from '@/types/necklaces';
...
interface NecklaceListingProps {
  initialNecklaces: Necklace[];
  totalCount: number;
}

const PRODUCTS_PER_PAGE = 12;

const NecklaceListing: React.FC<NecklaceListingProps> = ({
  initialNecklaces,
  totalCount,
}) => {
  const [necklaces, setNecklaces] = useState<Necklace[]>(initialNecklaces);
  ...
  const [sortBy, setSortBy] = useState<NecklacesSortBy>('NEW ARRIVALS');
  ...
  const [filters, setFilters] = useState<NecklaceFilters>({
  ...
  const sortOptions = NECKLACE_SORT_OPTIONS;
  ...
  // Fetch necklaces based on filters and sort
  const fetchNecklaces = useCallback(
    async (page: number = 1) => {
      setIsLoading(true);
      try {
        // Build query with proper escaping and error handling
        const queryConditions = ['_type == "necklace"', 'active == true'];
        ...
        // On error, keep the current necklaces but stop loading
        setIsLoading(false);
  ...
  }, [filters, sortBy, fetchNecklaces, currentPage]);

  const handleFilterChange = (
    filterType: keyof NecklaceFilters,
    value: NecklaceFilters[keyof NecklaceFilters]
  ) => {
    setFilters((prev) => ({
      ...prev,
      [filterType]: value,
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="text-sm text-gray-600 mb-4">
        <span>Home</span> / <span>All Necklaces</span>
      </div>

      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">Necklaces</h1>
        <p className="text-gray-600">
          Find your perfect necklace, Necklaces for every style, every occasion.
        </p>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="hidden lg:grid lg:grid-cols-7 gap-2">
          {NECKLACE_CATEGORIES.map((category) => {
        ...
        <div className="lg:hidden relative">
          ...
          >
            {NECKLACE_CATEGORIES.map((category, index) => {
              const isActive = filters.category?.includes(category) || false;
              return (
                <div
                  key={category}
                  ...
                  style={{
                    width: '120px',
                    marginRight:
                      index === NECKLACE_CATEGORIES.length - 1 ? '16px' : '0px',
                  }}
                  onClick={() => handleArrayFilterChange('category', category)}
                >
                  <div className="aspect-square rounded-lg overflow-hidden bg-gray-200">
              ...
              <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-gray-300 rounded-md shadow-lg z-50">
                {sortOptions.map((option) => (
                  <button
                    key={option}
                    onClick={() => {
                      setSortBy(option as NecklacesSortBy);
                      setIsSortOpen(false);
                    }}
          ...
          <div className="w-64 flex-shrink-0">
            <div className="bg-white">
              ...
              <div className="mb-6">
                ...
                {expandedFilters.material && (
                  <div className="mt-3 space-y-2">
                    {NECKLACE_MATERIALS.map((material) => (
                      <label
                        key={material}
              ...
              <div className="mb-6">
                ...
                {expandedFilters.price && (
                  <div className="mt-3 space-y-2">
                    {NECKLACE_PRICE_RANGES.map((range) => (
                      <label
                        key={range}
                        className="flex items-center space-x-2"
                      >
                    ...
                    <Link href={`/necklaces/${necklace.slug.current}`}>
                      <div className="cursor-pointer">
                        ...
                        <p className="text-gray-600 text-sm mb-2">
                          {necklace.material}, {necklace.stone}, Size{' '}
                          {necklace.size}
                        </p>
