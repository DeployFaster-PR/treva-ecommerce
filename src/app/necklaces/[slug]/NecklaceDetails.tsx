...
interface NecklaceDetailsProps {
  necklace: Necklace;
}

const NecklaceDetails: React.FC<NecklaceDetailsProps> = ({ necklace }) => {
  ...
  if (necklace.mainImageUrl) {
    allImages.push(necklace.mainImageUrl);
  }
  if (necklace.imageUrls && Array.isArray(necklace.imageUrls)) {
    necklace.imageUrls.forEach((url) => {
      if (
        url &&
        typeof url === 'string' &&
        url.trim() !== '' &&
  ...
  const averageRating = necklace.averageRating || 0;
  const totalReviews = necklace.totalReviews || 0;
  const hasReviews = necklace.reviews && necklace.reviews.length > 0;

  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      {/* Breadcrumb */}
      <div className="text-sm text-gray-900 mb-6">
        <span>Home</span> / <span>Necklaces</span> /{' '}
      ...
      <div className="flex flex-col lg:flex-row gap-4 lg:gap-8 mb-12">
              ...
              {allImages.length > 0 ? (
                <div className="w-full h-96 lg:h-[500px] bg-gray-100 rounded-lg overflow-hidden">
                  <Image
                    ...
                    alt={`${necklace.name} view ${selectedImage + 1}`}
                    width={600}
                    height={600}
                    className="w-full h-full object-cover"
        ...
        <div className="flex-1 lg:w-2/5 space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mb-4">
              <span className="text-xl lg:text-2xl font-bold text-gray-900">
                {formatPrice(necklace.price, necklace.currency)}
              </span>
              {necklace.originalPrice && (
                <span className="text-lg lg:text-xl text-gray-500 line-through">
                  {formatPrice(necklace.originalPrice, necklace.currency)}
                </span>
              )}
            </div>
            ...
            <p className="text-gray-900 text-lg mb-4">
              {necklace.material}, {necklace.stone} - Size {necklace.size}
            </p>
          </div>
          ...
          <div className="bg-gray-100 p-4 rounded-lg">
            ...
            {expandedSections.size && (
              <div className="mt-3">
                <span className="text-gray-900">Size {necklace.size}</span>
              </div>
            )}
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-900">
                {necklace.freeShipping || necklace.price >= 150
                  ? '🚚 Free shipping'
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-900">
                {necklace.warranty || '↩️ 3 year warranty'}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-900">
                {necklace.shippingInfo?.returnPolicy || '🔄 30 Day Returns'}
              </span>
            </div>
          </div>
            ...
            <div className="border-b-2 border-gray-400">
              ...
              {expandedSections.description && (
                <div className="pb-4 transition-all duration-300">
                  <p className="text-gray-900 leading-relaxed">
                    {necklace.description || 'No description available.'}
                  </p>
                </div>
              )}
            </div>
                ...
                <div className="pb-4 space-y-2 transition-all duration-300">
                  {necklace.dimensions?.length && (
                    <p className="text-sm text-gray-900">
                      <strong>Length:</strong> {necklace.dimensions.length}
                    </p>
                  )}
                  {necklace.dimensions?.width && (
                    <p className="text-sm text-gray-900">
                      <strong>Width:</strong> {necklace.dimensions.width}
                    </p>
                  )}
                  {necklace.dimensions?.weight && (
                    <p className="text-sm text-gray-900">
                      <strong>Weight:</strong> {necklace.dimensions.weight}
                    </p>
                  )}
                  <p className="text-sm text-gray-900">
                    <strong>Size:</strong> {necklace.size}
                  </p>
                  <p className="text-sm text-gray-900">
                    <strong>Color:</strong> {necklace.color}
                  </p>
                  <p className="text-sm text-gray-900">
                    <strong>Category:</strong> {necklace.category}
                  </p>
                </div>
              )}
            </div>
              ...
              {expandedSections.materials && (
                <div className="pb-4 transition-all duration-300">
                  <p className="text-sm text-gray-900 mb-2">
                    <strong>Material:</strong> {necklace.material}
                  </p>
                  <p className="text-sm text-gray-900 mb-2">
                    <strong>Stone:</strong> {necklace.stone}
                  </p>
