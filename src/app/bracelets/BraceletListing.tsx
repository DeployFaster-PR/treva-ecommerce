...
import {
  ...
  BRACELET_COLORS,
  BRACELET_SIZES,
  BRACELET_PRICE_RANGES,
  BRACELET_SORT_OPTIONS,
  CATEGORY_IMAGES,
...
interface BraceletListingProps {
  initialBracelets: Bracelet[];
  totalCount: number;
}

const PRODUCTS_PER_PAGE = 12;

const BraceletListing: React.FC<BraceletListingProps> = ({
  initialBracelets,
  totalCount,
}) => {
  const [bracelets, setBracelets] = useState<Bracelet[]>(initialBracelets);
  ...
  const [sortBy, setSortBy] = useState<BraceletsSortBy>('NEW ARRIVALS');
  ...
  const [filters, setFilters] = useState<BraceletFilters>({
  ...
  const sortOptions = BRACELET_SORT_OPTIONS;
  ...
  // Fetch bracelets based on filters and sort
  const fetchBracelets = useCallback(
    async (page: number = 1) => {
      setIsLoading(true);
      try {
        // Build query with proper escaping and error handling
        const queryConditions = ['_type == "bracelet"', 'active == true'];
        ...
        // On error, keep the current bracelets but stop loading
        setIsLoading(false);
  ...
  }, [filters, sortBy, fetchBracelets, currentPage]);

  const handleFilterChange = (
    filterType: keyof BraceletFilters,
    value: BraceletFilters[keyof BraceletFilters]
  ) => {
    setFilters((prev) => ({
      ...prev,
      [filterType]: value,
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="text-sm text-gray-600 mb-4">
        <span>Home</span> / <span>All Bracelets</span>
      </div>

      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">Bracelets</h1>
        <p className="text-gray-600">
          Find your perfect bracelet, Bracelets for every style, every occasion.
        </p>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="lg:hidden relative">
          ...
          >
            {BRACELET_CATEGORIES.map((category, index) => {
              const isActive = filters.category?.includes(category) || false;
              return (
                <div
                  key={category}
                  ...
                  style={{
                    width: '120px',
                    marginRight:
                      index === BRACELET_CATEGORIES.length - 1 ? '16px' : '0px',
                  }}
                  onClick={() => handleArrayFilterChange('category', category)}
                >
                  <div className="aspect-square rounded-lg overflow-hidden bg-gray-200">
              ...
              <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-gray-300 rounded-md shadow-lg z-50">
                {sortOptions.map((option) => (
                  <button
                    key={option}
                    onClick={() => {
                      setSortBy(option as BraceletsSortBy);
                      setIsSortOpen(false);
                    }}
          ...
          <div className="w-64 flex-shrink-0">
            <div className="bg-white">
              ...
              <div className="mb-6">
                ...
                {expandedFilters.material && (
                  <div className="mt-3 space-y-2">
                    {BRACELET_MATERIALS.map((material) => (
                      <label
                        key={material}
              ...
              <div className="mb-6">
                ...
                {expandedFilters.price && (
                  <div className="mt-3 space-y-2">
                    {BRACELET_PRICE_RANGES.map((range) => (
                      <label
                        key={range}
                        className="flex items-center space-x-2"
                      >
                    ...
                    <Link href={`/bracelets/${bracelet.slug.current}`}>
                      <div className="cursor-pointer">
                        ...
                        <p className="text-gray-600 text-sm mb-2">
                          {bracelet.material}, {bracelet.stone}, Size{' '}
                          {bracelet.size}
                        </p>
          ...
          {bracelets.length === 0 && !isLoading && (
            <div className="text-center py-12">
              <p className="text-gray-500">
                No bracelets found matching your criteria.
              </p>
            </div>
          )}
...
export default BraceletListing;
