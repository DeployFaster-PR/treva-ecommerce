...
interface BraceletDetailsProps {
  bracelet: Bracelet;
}

const BraceletDetails: React.FC<BraceletDetailsProps> = ({ bracelet }) => {
  ...
  if (bracelet.mainImageUrl) {
    allImages.push(bracelet.mainImageUrl);
  }
  if (bracelet.imageUrls && Array.isArray(bracelet.imageUrls)) {
    bracelet.imageUrls.forEach((url) => {
      if (
        url &&
        typeof url === 'string' &&
        url.trim() !== '' &&
  ...
  const averageRating = bracelet.averageRating || 0;
  const totalReviews = bracelet.totalReviews || 0;
  const hasReviews = bracelet.reviews && bracelet.reviews.length > 0;

  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      {/* Breadcrumb */}
      <div className="text-sm text-gray-900 mb-6">
        <span>Home</span> / <span>Bracelets</span> /{' '}
              ...
              {allImages.length > 0 &&
                allImages.map((image, index) => (
                  ...
                  >
                    <Image
                      src={image}
                      alt={`${bracelet.name} view ${index + 1}`}
                      width={80}
                      height={80}
              ...
              {allImages.length > 0 ? (
                <div className="w-full h-96 lg:h-[500px] bg-gray-100 rounded-lg overflow-hidden">
                  <Image
                    ...
                    alt={`${bracelet.name} view ${selectedImage + 1}`}
                    width={600}
                    height={600}
                    className="w-full h-full object-cover"
        ...
        <div className="flex-1 lg:w-2/5 space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              {bracelet.originalPrice && (
                <span className="text-lg lg:text-xl text-gray-500 line-through">
                  {formatPrice(bracelet.originalPrice, bracelet.currency)}
                </span>
              )}
            </div>
            ...
            <p className="text-gray-900 text-lg mb-4">
              {bracelet.material}, {bracelet.stone} - Size {bracelet.size}
            </p>
          </div>
          ...
          <div className="bg-gray-100 p-4 rounded-lg">
            ...
            {expandedSections.size && (
              <div className="mt-3">
                <span className="text-gray-900">Size {bracelet.size}</span>
              </div>
            )}
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-900">
                {bracelet.freeShipping || bracelet.price >= 150
                  ? '🚚 Free shipping'
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-900">
                {bracelet.warranty || '↩️ 3 year warranty'}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-900">
                {bracelet.shippingInfo?.returnPolicy || '🔄 30 Day Returns'}
              </span>
            </div>
          </div>
            ...
            <div className="border-b-2 border-gray-400">
              ...
              {expandedSections.description && (
                <div className="pb-4 transition-all duration-300">
                  <p className="text-gray-900 leading-relaxed">
                    {bracelet.description || 'No description available.'}
                  </p>
                </div>
              )}
            </div>
                ...
                <div className="pb-4 space-y-2 transition-all duration-300">
                  {bracelet.dimensions?.length && (
                    <p className="text-sm text-gray-900">
                      <strong>Length:</strong> {bracelet.dimensions.length}
                    </p>
                  )}
                  {bracelet.dimensions?.width && (
                    <p className="text-sm text-gray-900">
                      <strong>Width:</strong> {bracelet.dimensions.width}
                    </p>
                  )}
                  {bracelet.dimensions?.weight && (
                    <p className="text-sm text-gray-900">
                      <strong>Weight:</strong> {bracelet.dimensions.weight}
                    </p>
                  )}
                  <p className="text-sm text-gray-900">
                  ...
                  <p className="text-sm text-gray-900">
                    <strong>Color:</strong> {bracelet.color}
                  </p>
                  <p className="text-sm text-gray-900">
                    <strong>Category:</strong> {bracelet.category}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="border-b-2 border-gray-400">
              ...
              {expandedSections.materials && (
                <div className="pb-4 transition-all duration-300">
                  ...
                  <p className="text-sm text-gray-900 mb-2">
                    <strong>Stone:</strong> {bracelet.stone}
                  </p>
