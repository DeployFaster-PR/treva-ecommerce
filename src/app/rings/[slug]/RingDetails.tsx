...
const RingDetails: React.FC<RingDetailsProps> = ({ ring }) => {
  ...
  if (ring.mainImageUrl) {
    allImages.push(ring.mainImageUrl);
  }
  if (ring.imageUrls && Array.isArray(ring.imageUrls)) {
    ring.imageUrls.forEach((url) => {
      if (
        url &&
        typeof url === 'string' &&
        url.trim() !== '' &&
  ...
  const averageRating = ring.averageRating || 0;
  const totalReviews = ring.totalReviews || 0;
  const hasReviews = ring.reviews && ring.reviews.length > 0;

  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      {/* Breadcrumb */}
      <div className="text-sm text-gray-900 mb-6">
        <span>Home</span> / <span>Rings</span> / <span>{ring.name}</span>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4 lg:gap-8 mb-12">
              ...
              {allImages.length > 0 ? (
                <div className="w-full h-96 lg:h-[500px] bg-gray-100 rounded-lg overflow-hidden">
                  <Image
                    ...
                    alt={`${ring.name} view ${selectedImage + 1}`}
                    width={600}
                    height={600}
                    className="w-full h-full object-cover"
        ...
        <div className="flex-1 lg:w-2/5 space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mb-4">
              <span className="text-xl lg:text-2xl font-bold text-gray-900">
                {formatPrice(ring.price, ring.currency)}
              </span>
              {ring.originalPrice && (
                <span className="text-lg lg:text-xl text-gray-500 line-through">
                  {formatPrice(ring.originalPrice, ring.currency)}
                </span>
              )}
            </div>
          ...
          <div className="bg-gray-100 p-4 rounded-lg">
            ...
            {expandedSections.size && (
              <div className="mt-3">
                <span className="text-gray-900">Size {ring.size}</span>
              </div>
            )}
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-900">
                {ring.freeShipping || ring.price >= 150
                  ? '🚚 Free shipping'
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-900">
                {ring.warranty || '↩️ 3 year warranty'}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-900">
                {ring.shippingInfo?.returnPolicy || '🔄 30 Day Returns'}
              </span>
            </div>
          </div>
            ...
            <div className="border-b-2 border-gray-400">
              ...
              {expandedSections.description && (
                <div className="pb-4 transition-all duration-300">
                  <p className="text-gray-900 leading-relaxed">
                    {ring.description || 'No description available.'}
                  </p>
                </div>
              )}
            </div>
                ...
                <div className="pb-4 space-y-2 transition-all duration-300">
                  {ring.dimensions?.length && (
                    <p className="text-sm text-gray-900">
                      <strong>Length:</strong> {ring.dimensions.length}
                    </p>
                  )}
                  {ring.dimensions?.width && (
                    <p className="text-sm text-gray-900">
                      <strong>Width:</strong> {ring.dimensions.width}
                    </p>
                  )}
                  {ring.dimensions?.weight && (
                    <p className="text-sm text-gray-900">
                      <strong>Weight:</strong> {ring.dimensions.weight}
                    </p>
                  )}
                  <p className="text-sm text-gray-900">
                    <strong>Size:</strong> {ring.size}
                  </p>
                  <p className="text-sm text-gray-900">
                    <strong>Color:</strong> {ring.color}
                  </p>
                  <p className="text-sm text-gray-900">
                    <strong>Category:</strong> {ring.category}
                  </p>
                </div>
              )}
            </div>
              ...
              {expandedSections.materials && (
                <div className="pb-4 transition-all duration-300">
                  <p className="text-sm text-gray-900 mb-2">
                    <strong>Material:</strong> {ring.material}
                  </p>
                  <p className="text-sm text-gray-900 mb-2">
                    <strong>Stone:</strong> {ring.stone}
                  </p>
                  {ring.careInstructions && (
                    <p className="text-sm text-gray-900">
